...
export class VideoService {
  ...
  public async userGetDetails(videoId: string, currentUser: UserDto, jwToken: string): Promise<VideoDto> {
    ...
    const [performer, files, participants, reactions] = await Promise.all([
      ...
      video.participantIds.length ? await this.performerService.findByIds(participantIds) : [],
      this.reactionService.findByQuery({ objectType: REACTION_TYPE.VIDEO, objectId: video._id, createdBy: currentUser?._id })
    ]);
    ...
    dto.isBookmarked = !!reactions.filter((r) => r.action === REACTION.BOOKMARK).length;
  ...
  public async updateInfo(id: string | Types.ObjectId, payload: VideoUpdatePayload, files: any, updater: UserDto): Promise<VideoDto> {

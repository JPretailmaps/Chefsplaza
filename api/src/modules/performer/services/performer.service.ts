...
import {
  PERFORMER_UPDATE_STATUS_CHANNEL, DELETE_PERFORMER_CHANNEL, PERFORMER_STATUSES
...
export class PerformerService {
  ...
  public async findByUsername(
    ...
    let isBlocked = false;
    if (countryCode && `${user?._id}` !== `${model._id}` && !user?.isPerformer) {
      isBlocked = await this.performerBlockService.checkBlockedCountryByIp(model._id, countryCode);
      if (isBlocked) {
        throw new HttpException('Your country has been blocked by this creator', 403);
      }
    }
    ...
    let isFollowed = null;
    if (user) {
      isBlockedByPerformer = `${user?._id}` !== `${model._id}` && !user?.isPerformer && await this.performerBlockService.checkBlockedByPerformer(
        model._id,
        user._id
      );
      if (isBlockedByPerformer) throw new HttpException('You has been blocked by this creator', 403);
      isBookMarked = await this.reactionService.findOneQuery({
        objectType: REACTION_TYPE.PERFORMER, objectId: model._id, createdBy: user._id, action: REACTION.BOOKMARK
      });
  ...
  public async create(
    ...
    data.isFreeSubscription = SettingService.getValueByKey(SETTING_KEYS.FREE_SUBSCRIPTION_ENABLED) || true;
    data.durationFreeSubscriptionDays = SettingService.getValueByKey(SETTING_KEYS.FREE_SUBSCRIPTION_DURATION) || 1;
  ...
  public async socialCreate(payload: PerformerRegisterPayload): Promise<PerformerModel> {
    const data = omit({
      ...payload,
      updatedAt: new Date(),
      createdAt: new Date()
    ...
    if (data.username) {
      const [countPerformerUsername, countUserUsername] = await Promise.all([
        this.checkExistedEmailorUsername({ username: data.username }),
        this.userService.checkExistedEmailorUsername({ username: data.username })
      ]);
      if (countPerformerUsername || countUserUsername) {
        data.username = `creator-${new Date().getTime()}`;
      }
    }
    if (!data.name) {
  ...
  public async updateDocument(performerId: string | Types.ObjectId, file: FileDto, type: string) {
  ...
  public async updateAvatar(performerId: string | Types.ObjectId, file: FileDto) {
    const performer = await this.performerModel.findById(performerId);
    if (!performer) throw new EntityNotFoundException();
    await this.performerModel.updateOne(
      { _id: performerId },
  ...
  public async updateCover(performerId: string | Types.ObjectId, file: FileDto) {
    const performer = await this.performerModel.findById(performerId);
    if (!performer) throw new EntityNotFoundException();
    await this.performerModel.updateOne(
      { _id: performerId },
  ...
  public async updateWelcomeVideo(performerId: string | Types.ObjectId, file: FileDto) {
    const performer = await this.performerModel.findById(performerId);
    if (!performer) throw new EntityNotFoundException();
    await this.performerModel.updateOne(
      { _id: performerId },
  ...
  public async setStreamingStatus(id: string | Types.ObjectId, streamingStatus: string) {
  ...
  public async updateSubscriptionStat(performerId: string | Types.ObjectId, num = 1) {
    const performer = await this.performerModel.findById(performerId);
    if (!performer) return;
  ...
  public async updatePerformerBalance(performerId: string | Types.ObjectId, tokens: number) {

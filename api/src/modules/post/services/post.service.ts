...
export class PostService {
  ...
  public async findByIdOrSlug(id: string | Types.ObjectId): Promise<PostModel> {
    const query = id instanceof Types.ObjectId || StringHelper.isObjectId(id)
      ? { _id: id }
      : { slug: id };
    return this.postModel.findOne(query as any);
  }

  public async generateSlug(title: string, id?: string | Types.ObjectId) {

...
export class CategoryService {
  ...
  public async findByIdOrSlug(id: string | Types.ObjectId): Promise<CategoryModel> {
    const query = id instanceof Types.ObjectId || StringHelper.isObjectId(id)
      ? { _id: id }
      : { slug: id };
    return this.categoryModel.findOne(query as any);
  }

  public async generateSlug(type: string, title: string, id?: string | Types.ObjectId) {
  ...
  public async update(id: string | Types.ObjectId, payload: CategoryUpdatePayload, user?: UserDto): Promise<CategoryModel> {
  ...
  public async delete(id: string | Types.ObjectId | CategoryModel, user?: UserDto): Promise<void> {
    ...
    if (category.parentId) {
      ...
      for (const subCat of children) {
        // eslint-disable-next-line no-await-in-loop
        await this.delete(subCat);
      }
    }
  }
}

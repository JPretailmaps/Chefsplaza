...
export class TransactionEarningListener {
  ...
  public async handleListenEarningToken(
    ...
    newEarning.set('updatedAt', new Date());
    await newEarning.save();
    // update balance
    await this.updateBalance(newEarning.grossPrice, netPrice, newEarning);
    await this.notifyPerformerBalance(newEarning.performerId, netPrice);
  }
  ...
  private async updateBalance(userTokens: number, performerTokens: number, earning: EarningModel) {
    await Promise.all([
  ...
  private async notifyPerformerBalance(performerId: Types.ObjectId, token: number) {
    await this.socketUserService.emitToUsers(performerId, 'update_balance', {
      token
    });
  }
}

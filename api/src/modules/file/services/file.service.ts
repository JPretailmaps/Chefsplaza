...
import {
  QueueEventService, QueueEvent, getConfig, EntityNotFoundException
} from 'src/kernel';
import {
...
export class FileService {
  ...
  public async findByIds(ids: string[] | Types.ObjectId[]): Promise<FileDto[]> {
  ...
  public async createFromMulter(
    ...
    let server = Storage.DiskStorage;
    const thumbnails = [];

    if (options.uploadImmediately) {
      // create thumb
      if (multerData.mimetype.includes('image') && options.generateThumbnail) {
      ...
      // upload file
      if (checkS3Settings) {
        const buffer = multerData.mimetype.includes('image') ? await this.imageService.replaceWithoutExif(multerData.path, multerData.mimetype) : readFileSync(multerData.path);
  ...
  private async _processVideo(event: QueueEvent) {
    ...
    try {
      ...
      const { width = 500, height = 500 } = videoMeta || {};
      ...
      if (checkS3Settings) {
        ...
        thumbnails = respThumb.map((name) => ({
          ...
          path: toPosixPath(join(videoDir, name).replace(publicDir, ''))
        }));
  ...
  private async _processPhoto(event: QueueEvent) {
    ...
    try {
      ...
      if (checkS3Settings) {
        ...
        thumbnailPath = toPosixPath(join(photoDir, thumbName).replace(publicDir, ''));

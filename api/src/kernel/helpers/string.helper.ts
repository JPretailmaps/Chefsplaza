...
export const getFileName = (fullPath: string, removeExtension: boolean) => {
  /* eslint-disable */
  const name = fullPath.replace(/[\s~`!@#$%^&*(){}\[\];:"'<,>?\/\\|+=]/g, '');
...
export const isObjectId = (id: string | mongoose.Types.ObjectId): boolean => /^[0-9a-fA-F]{24}$/.test(`${id}`);

export const toObjectId = (id: string | mongoose.Types.ObjectId) => new mongoose.Types.ObjectId(id as any);
